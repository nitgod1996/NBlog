<template>
    <div ref="nav" class="ui fixed inverted stackable pointing menu" :class="{ transparent: $route.name === 'home' && clientSize.clientWidth > 768 }">
        <div class="ui container">
            ...
            <router-link to="/home" class="item" :class="{ 'm-mobile-hide': mobileHide, active: $route.name === 'home' }"> <i class="home icon"></i>首页 </router-link>
            <el-dropdown trigger="click" @command="categoryRoute">
                <span class="el-dropdown-link item" :class="{ 'm-mobile-hide': mobileHide, active: $route.name === 'category' }"> <i class="idea icon"></i>分类<i class="caret down icon"></i> </span>
            ...
            <router-link to="/archives" class="item" :class="{ 'm-mobile-hide': mobileHide, active: $route.name === 'archives' }"> <i class="clone icon"></i>归档 </router-link>
            <router-link to="/moments" class="item" :class="{ 'm-mobile-hide': mobileHide, active: $route.name === 'moments' }"> <i class="comment alternate outline icon"></i>动态 </router-link>
            <!-- <router-link to="/friends" class="item" :class="{'m-mobile-hide': mobileHide,'active':$route.name==='friends'}">
				<i class="users icon"></i>友人帐
			</router-link> -->
            <router-link to="/about" class="item" :class="{ 'm-mobile-hide': mobileHide, active: $route.name === 'about' }"> <i class="info icon"></i>关于我 </router-link>
...
export default {
    ...
    methods: {
        ...
        querySearchAsync(queryString, callback) {
            if (queryString == null || queryString.trim() === '' || queryString.indexOf('%') !== -1 || queryString.indexOf('_') !== -1 || queryString.indexOf('[') !== -1 || queryString.indexOf('#') !== -1 || queryString.indexOf('*') !== -1 || queryString.trim().length > 20) {
                return
            }
